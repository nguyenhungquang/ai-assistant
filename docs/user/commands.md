...
When the staged packet includes extracted figures or equations, the draft output must include `media_review`. Select important `figure_ids` / `equation_ids` in the sections that explain them, or set `media_review.no_media_reason` if no extracted media should be included. Use equation `math_id` values in `equation_ids`, not display labels such as `Equation 1`. Older cached draft JSON without `media_review` must be regenerated or amended before finalization.
